...
const onStartup = async (context) => {
    try {
        ...
        if (noActiveWorkspace) {
            ...
            (0, commands_1.send)({ type: 'NO_WORKSPACE', payload: { error } });
            return;
        }
        const env = {
        ...
        if (!tutorial || !tutorial.id) {
            if (!!environment_1.TUTORIAL_URL) {
                try {
                    const tutorialRes = await (0, node_fetch_1.default)(environment_1.TUTORIAL_URL);
                    const tutorial = await tutorialRes.json();
                    (0, commands_1.send)({ type: 'START_TUTORIAL_FROM_URL', payload: { tutorial } });
                    return;
                }
                catch (e) {
            ...
            (0, commands_1.send)({ type: 'START_NEW_TUTORIAL', payload: { env } });
            return;
        }
        const { position } = await context.onContinue(tutorial);
        (0, commands_1.send)({ type: 'LOAD_STORED_TUTORIAL', payload: { env, tutorial, position } });
    }
    catch (e) {
        const error = {
            type: 'UnknownError',
        ...
        (0, commands_1.send)({ type: 'EDITOR_STARTUP_FAILED', payload: { error } });
    }
};
