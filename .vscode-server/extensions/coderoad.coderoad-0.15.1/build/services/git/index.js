...
const stashAllFiles = async () => {
    const { stderr } = await (0, node_1.exec)({ command: `git stash --include-untracked` });
    if (stderr) {
...
const cherryPickCommit = async (commit, count = 0) => {
    ...
    try {
        const { stdout } = await (0, node_1.exec)({ command: `git cherry-pick -X theirs ${commit}` });
        if (!stdout) {
...
async function saveCommit(message) {
    const { stdout, stderr } = await (0, node_1.exec)({ command: `git commit -am '${message}'` });
    if (stderr) {
        console.error(stderr);
        throw new Error('Error saving progress to Git');
    }
    (0, logger_1.default)(['save with commit & continue stdout', stdout]);
}
exports.saveCommit = saveCommit;
async function clear() {
    try {
        const { stderr } = await (0, node_1.exec)({ command: 'git reset HEAD --hard && git clean -fd' });
        if (!stderr) {
            return;
        }
        console.error(stderr);
...
exports.clear = clear;
async function init() {
    const { version: gitVersion, error: gitError } = await (0, dependencies_1.getVersion)('git');
    if (gitError) {
    ...
    const hasInitialBranch = await (0, dependencies_1.compareVersions)(gitVersion, '>=2.28.0');
    let stderr;
    if (hasInitialBranch) {
        const initResult = await (0, node_1.exec)({ command: 'git init --initial-branch=master' });
        stderr = initResult.stderr;
    }
    else {
        const initResult = await (0, node_1.exec)({ command: 'git init' });
        stderr = initResult.stderr;
    }
    if (stderr) {
...
async function initIfNotExists() {
    const hasGitInit = (0, node_1.exists)('.git');
    if (!hasGitInit) {
        await init();
    }
}
...
async function checkRemoteConnects(repo) {
    const externalRepoExists = await (0, node_1.exec)({ command: `git ls-remote --exit-code --heads ${repo.uri}` });
    if (externalRepoExists.stderr) {
        throw new Error(externalRepoExists.stderr);
    }
    const { stderr, stdout } = await (0, node_1.exec)({ command: `git ls-remote --exit-code --heads ${repo.uri} ${repo.branch}` });
    if (stderr) {
...
async function addRemote(repo) {
    const { stderr } = await (0, node_1.exec)({ command: `git remote add ${exports.gitOrigin} ${repo} && git fetch ${exports.gitOrigin}` });
    if (stderr) {
...
async function checkRemoteExists() {
    try {
        const { stdout, stderr } = await (0, node_1.exec)({ command: 'git remote -v' });
        if (stderr) {
            return false;
        }
...
async function loadCommitHistory() {
    try {
        const { stdout, stderr } = await (0, node_1.exec)({ command: 'git log --pretty=format:"%h"' });
        if (stderr) {
            return [];
        }
        return stdout.split('\n');
...
async function getCommitMessage(hash) {
    try {
        const { stdout, stderr } = await (0, node_1.exec)({ command: `git log -n 1 --pretty=format:%s ${hash}` });
        if (stderr) {
            return null;
        }
        return stdout;
...
async function commitsExistsByMessage(message) {
    try {
        const { stdout, stderr } = await (0, node_1.exec)({ command: `git log -g --grep='${message}'` });
        if (stderr) {
            return false;
        }
...
async function validateGitConfig(target) {
    try {
        const { stdout, stderr } = await (0, node_1.exec)({ command: `git config ${target}` });
        if (stderr) {
            return false;
        }
        return !!stdout.length;
