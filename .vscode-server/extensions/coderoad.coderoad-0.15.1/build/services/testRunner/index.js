...
const createTestRunner = (data, callbacks) => {
    ...
    return async ({ position, onSuccess }) => {
        var _a;
        const startTime = (0, throttle_1.throttle)();
        if (!startTime) {
            return;
        }
        (0, logger_1.default)('------------------- RUN TEST -------------------');
        ...
        try {
            ...
            (0, logger_1.default)('COMMAND', command);
            result = await (0, node_1.exec)({ command, dir: testRunnerConfig.directory });
        }
        catch (err) {
            result = { stdout: err.stdout, stderr: err.stack };
        }
        if (!(0, throttle_1.debounce)(startTime)) {
            return;
        }
        (0, logger_1.default)('----------------- PROCESS TEST -----------------');
        const { stdout, stderr } = result;
        const tap = (0, parser_1.default)(stdout || '');
        (0, output_1.addOutput)({ channel: logChannelName, text: tap.logs.join('\n'), show: false });
        if (stderr) {
            if (!tap.failed.length) {
                ...
                if (step.subtasks) {
                    const subtaskSummary = (0, subtasks_1.default)(tap.summary, position.stepId || '');
                ...
                const output = (0, formatOutput_1.formatFailOutput)(tap);
                (0, output_1.addOutput)({ channel: failChannelName, text: output, show: true });
                return;
            }
            else {
                callbacks.onError(position);
                (0, output_1.addOutput)({ channel: failChannelName, text: stderr, show: true });
                return;
            }
        }
        if (tap.ok) {
            (0, output_1.clearOutput)(failChannelName);
            ...
            (0, telemetry_1.onError)(new Error(`Error with running test ${JSON.stringify(position)}`));
            callbacks.onError(position);
        }
    };
};
